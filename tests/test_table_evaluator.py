...
def test_initialization(sample_data: tuple[pd.DataFrame, pd.DataFrame]):
...
def test_correlation_distance(sample_data: tuple[pd.DataFrame, pd.DataFrame]):
    real, fake = sample_data
    evaluator = TableEvaluator(real, fake)

    with patch('table_evaluator.table_evaluator.te_metrics.euclidean_distance') as mock_distance:
...
def test_get_copies(sample_data: tuple[pd.DataFrame, pd.DataFrame]):
    real, fake = sample_data
...
def test_get_copies_no_matches(sample_data: tuple[pd.DataFrame, pd.DataFrame]):
    real, fake = sample_data
...
def test_get_duplicates(sample_data: tuple[pd.DataFrame, pd.DataFrame]):
    real, fake = sample_data
    evaluator = TableEvaluator(real, fake)
...
def test_pca_correlation(sample_data: tuple[pd.DataFrame, pd.DataFrame]):
    real, fake = sample_data
    evaluator = TableEvaluator(real, fake)
...
def test_basic_statistical_evaluation(sample_data: tuple[pd.DataFrame, pd.DataFrame]):
    real, fake = sample_data
    evaluator = TableEvaluator(real, fake)
...
def test_correlation_correlation(sample_data: tuple[pd.DataFrame, pd.DataFrame]):
    real, fake = sample_data
    evaluator = TableEvaluator(real, fake)
...
def test_convert_numerical(sample_data: tuple[pd.DataFrame, pd.DataFrame]):
    real, fake = sample_data
    evaluator = TableEvaluator(real, fake)
...
def test_convert_numerical_one_hot(sample_data: tuple[pd.DataFrame, pd.DataFrame]):
    real, fake = sample_data
    evaluator = TableEvaluator(real, fake, verbose=True)
...
def test_estimator_evaluation(sample_data: tuple[pd.DataFrame, pd.DataFrame]):
    real, fake = sample_data
    evaluator = TableEvaluator(real, fake, verbose=True)

    with patch.object(evaluator.ml_evaluator, 'estimator_evaluation', return_value=0.8) as mock_ml_eval:
...
def test_row_distance(sample_data: tuple[pd.DataFrame, pd.DataFrame]):
    real, fake = sample_data
    evaluator = TableEvaluator(real, fake)
...
def test_column_correlations(sample_data: tuple[pd.DataFrame, pd.DataFrame]):
    real, fake = sample_data
    evaluator = TableEvaluator(real, fake)

    with patch('table_evaluator.table_evaluator.te_metrics.column_correlations') as mock_column_correlations:
        mock_column_correlations.return_value = 0.75
        result = evaluator.column_correlations()
        assert result == 0.75


def test_evaluate(sample_data: tuple[pd.DataFrame, pd.DataFrame]):
    real, fake = sample_data
    evaluator = TableEvaluator(real, fake)
...
def test_visual_evaluation(sample_data: tuple[pd.DataFrame, pd.DataFrame]):
    real, fake = sample_data
    evaluator = TableEvaluator(real, fake)

    with patch.object(evaluator.visualization_manager, 'visual_evaluation') as mock_visual_eval:
        evaluator.visual_evaluation(show=False)
        mock_visual_eval.assert_called_once_with(save_dir=None, show=False)


def test_error_handling():
    with pytest.raises(ValueError, match='real and fake DataFrames must have the same columns'):
        TableEvaluator(

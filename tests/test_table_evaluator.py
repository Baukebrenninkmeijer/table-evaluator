...
def test_pca_correlation(sample_data):
    ...
    with patch('table_evaluator.table_evaluator.PCA') as mock_pca, \
         patch.object(evaluator, 'comparison_metric', return_value=(0.8, 0.1, 0.1)) as mock_comp_metric:
        mock_pca().explained_variance_ = np.array([0.5, 0.3, 0.2])
...
def test_estimator_evaluation(sample_data):
    ...
    with patch('table_evaluator.table_evaluator.KFold') as mock_kfold, \
...
def test_evaluate(sample_data):
    ...
    with patch.object(TableEvaluator, 'basic_statistical_evaluation', return_value=0.8), \
...
def test_visual_evaluation(sample_data):
    ...
    with patch.object(evaluator, 'plot_mean_std') as mock_mean_std, \

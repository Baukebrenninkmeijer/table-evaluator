...
def test_euclidean_distance():
    np.testing.assert_almost_equal(te_metrics.euclidean_distance([0, 0], [1, 1]), 1.41421356)
...
def test_associations():
    ...
    pd.testing.assert_frame_equal(
        real_assoc_theil,
        associations(real, nominal_columns=cat_cols, nom_nom_assoc='theil', compute_only=True)['corr'],
    )
    pd.testing.assert_frame_equal(
        fake_assoc,
    ...
    pd.testing.assert_frame_equal(
        fake_assoc_theil,
        associations(fake, nominal_columns=cat_cols, nom_nom_assoc='theil', compute_only=True)['corr'],
    )
